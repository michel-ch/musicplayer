...
[GitHub Releases page](https://github.com/michel-ch/musicplayer/releases); the full history
lives in [`CHANGELOG.md`](CHANGELOG.md). The current version is **1.1.13** (`versionCode` 10).

Each release maps to a `vX.Y.Z` git tag. There is no Play Store build yet — build the APK as
above (or grab it from a release once attached) and sideload it. The release APK is signed with
the standard Android debug keystore.
...
[`docs/architecture-fluxes.md`](docs/architecture-fluxes.md) (the master list).
