...
`BluetoothReceiver` is registered with `RECEIVER_EXPORTED` (required on API 33+ for the OS-sent `ACTION_ACL_CONNECTED` / `ACTION_HEADSET_PLUG` broadcasts; `RECEIVER_NOT_EXPORTED` silently drops them). On a connect event it resumes only when `SettingsViewModel.AUTO_RESUME_HEADSET_KEY` is on (default off) **and** the player isn't user-paused. Reading a device's `bluetoothClass` needs `BLUETOOTH_CONNECT`, which `MainActivity` requests at runtime on API 31+ (a denial now raises a toast so the degraded auto-resume isn't silent). When the class **is** readable, only `AUDIO_VIDEO`-major devices resume. When it's **unreadable** (permission denied → `SecurityException`, or a missing device extra), the receiver no longer blindly assumes audio — it falls back to `hasBluetoothAudioOutput()`, an `AudioManager.getDevices(GET_DEVICES_OUTPUTS)` check for an actual BT audio output (needs no permission), so connecting a Bluetooth mouse/keyboard/watch can't wake playback. The device extra is read via `IntentCompat.getParcelableExtra` (the bare `getParcelableExtra` is deprecated on API 33+).
...
- `initialize(audioSessionId)` (called from the service's `AnalyticsListener`) rebuilds the hardware effects, restores base bands + preamp/tone scalars, then re-applies them — so an audio-session-id change no longer silently drops the user's EQ. The restore reads run in coroutines that suspend on DataStore; because rapid Bluetooth route changes can fire a second `initialize()` (which `release()`s the effects) while a restore is mid-flight, `restoreSettings`/`restoreBassBoost` re-check the captured `Equalizer`/`BassBoost` is still the live instance (`eq !== equalizer` → return) before touching it, guarding against an `IllegalStateException` crash on a released effect.
