            ...
            if (songs.isNotEmpty()) {
                item {
                    Row(
                        ...
                        Row {
                            IconButton(onClick = { viewModel.playAll(songs) }) { Icon(Icons.Default.PlayArrow, contentDescription = "Play all") }
                            IconButton(onClick = { viewModel.playAll(songs.shuffled()) }) { Icon(Icons.Default.Shuffle, contentDescription = "Shuffle") }
                        }
                    }
                }
            }
            items(songs, key = { it.id }) { song ->
                SongItem(song = song, isPlaying = song.id == playbackState.currentSong?.id, onClick = { viewModel.playSong(song, songs) }, onLongClick = { selectedSong = song })
            }
        }
    }
}
