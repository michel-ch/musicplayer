...
| File | Purpose |
|------|---------|
| `MainActivity.kt` | Activity entry point, permission handling, Hilt setup. Observes the `keep_screen_on` DataStore flag while STARTED and toggles `FLAG_KEEP_SCREEN_ON` on the window — flag is released when the activity is backgrounded so it never blocks system standby. `onResume` calls `maybeResumeOnForeground()` which checks the `resume_on_app_foreground` setting, asks `AudioManager.getDevices(GET_DEVICES_OUTPUTS)` whether a BT audio output (A2DP / SCO / BLE headset on API 31+) is connected, and resumes paused playback via the injected `PlaybackController`. The runtime-permission launcher requests `BLUETOOTH_CONNECT` on API 31+ and now toasts on denial so the degraded headset auto-resume isn't silent. |
...
| `BluetoothReceiver.kt` | Handles Bluetooth/headset connect events. Reads the device extra via `IntentCompat.getParcelableExtra`. Reading `bluetoothClass` is wrapped in try/catch (needs `BLUETOOTH_CONNECT`); a readable class resumes only for `AUDIO_VIDEO`-major devices, while an unreadable class (permission denied / missing extra) falls back to `hasBluetoothAudioOutput()` (`AudioManager.getDevices`, no permission) so a BT mouse/keyboard/watch can't wake playback. |
...
| `audio/EqualizerManager.kt` | Wraps Android AudioFX Equalizer + BassBoost, persists settings to DataStore. `applyEffects()` writes each band as base + preamp + tone in one pass (preamp/tone no longer overwrite each other); `initialize()` re-applies them after an audio-session-id change. The async `restoreSettings`/`restoreBassBoost` re-check the captured effect is still the live instance after their DataStore await, so a concurrent `initialize()`+`release()` (rapid BT route changes) can't make them touch a released effect (`IllegalStateException` crash guard). |
